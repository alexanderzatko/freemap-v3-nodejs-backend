...
function verifyTokenSignatureOrThrow(token: string, providedHex: string) {
  const expected = createHmac('sha256', getEnv('PURCHASE_SECRET'))
    .update(token)
    .digest();
...
function enforceFreshnessOrThrow(occurredAt: number, expiration?: number) {
  ...
  if (expiration !== undefined && occurredAt > expiration) {
...
export function attachPurchaseValidateHandler(router: RouterInstance) {
  ...
  router.post('/purchaseValidate', async (ctx) => {
    ...
    const webhookRes = WebhookSchema.safeParse(ctx.request.body);
        ...
        await conn.query<unknown>(
          sql`
            ...
            SET
              ...
              email = ${webhook.email},
              bankIntentStatus = ${webhook.event === 'payment-completed' ? null : webhook.bank_intent_status}
            WHERE
              token = ${webhook.token}
          `,
        );
    ...
    let legacy;
    try {
      legacy = LegacyBodySchema.parse(ctx.request.body);
    } catch (err) {
